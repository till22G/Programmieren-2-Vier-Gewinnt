...
public class Singleplayer implements ActionListener {		//implementieren, um Aktionen per Mausklick festzulegen
	...
	public int vorbei = 0;								//flag fuer Spielende nur fuer KI, damit kein Stein mehr erstellt wird bei Neustart
	...
	public Singleplayer (){									
		for (int i=0;i<6;i++) {
    		for(int j=0;j<7;j++) {
    			Spielfeld.feld[i][j].addActionListener(this);	//hinzufuegen der JButtons zu ActionListener, um diese ansprechen zu kuennen
    		}
    	}
				 ...
				 if (counter[i]>-1) {
						...
						while(counter[p]==-1) {
						 ...
						 counter[p]--;
							...
							checkWinH(); // horizontal Spieler 1
							checkWinV(); // vertikal Spieler 2
	...
	public void checkWinH() {				// horizontal
			for (int a = 0; a < 6; a++) { 
				gewinncounter = 0;
				for (int b = 0; b < 7; b++) {
	...
	public void checkWinV() {				// horizontal
	for (int a = 0; a < 7; a++) { 
		gewinncounter = 0;
		for (int b = 0; b < 6; b++) {
